...
// When hey-social adopts hey-chat, route() can be made pluggable to re-add
// its social arms.
        ...
        if let Some(s) = session::current() {
            if let Err(e) = poll_once(&s.did_key).await {
                web_sys::console::warn_1(&JsValue::from_str(&format!(
                    "[hey-chat] peer_receiver poll error: {e}"
                )));
            }
        }
    }
...
async fn consume_v2_queue(topic: &str, consumer_id: &str) {
    ...
    for entry in arr {
        ...
        if let Err(e) = dms::receive_v2_wire(topic, wire).await {
            web_sys::console::warn_1(&JsValue::from_str(&format!("[hey-chat] v2 dm consume: {e}")));
        }
    }
}
