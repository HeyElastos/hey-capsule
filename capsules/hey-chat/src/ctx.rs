...
//!
//! `hey-chat` is shared by hey-social and hey-messenger. The transport,
...
//! `hey_chat::ctx::init(CapsuleCtx { .. })` once in `main()` before mounting,
...
fn get() -> CapsuleCtx {
    CTX.with(|c| {
        *c.get().expect(
            "hey_chat::ctx::init(CapsuleCtx { .. }) must be called in main() before using the engine",
        )
    })
}
